...
export const selectPokemonList = createSelector( selectPokemons, selectTypes, ( pokemons, pokemonTypes ) =>
  pokemons?.map( ({ types, ...pokemon }) => ({      
    ...pokemon,
    types: types!.map( ( type ) => pokemonTypes.find( ({ id }) => id === type )?.name || type as TTypeName ),
  }))
);
